$(document).ready(function() {
    ...
    $("#imageUpload").change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url_for('Blur_Background_btn.upload_image') }}",
            data: formData,
            contentType: false,
            cache: false,
            processData: false,
            success: function(data) {
                ...
                image.src = '/Blur_Background_btn/get_original_image/' + filename;
                console.log('Success upload!');
            }
        });
    });

    $('#blur-button').on('click', function(event) {
        $('.loader').show();
        $.ajax({
            type: 'POST',
            url: '/Blur_Background_btn/blur',
            data: JSON.stringify({filename: filename, blur_ratio: inputv.value }),
            ...
            success: function(data) {
                ...
                var img = new Image();
                img.src = '/Blur_Background_btn/get_image/' + filename;
    ...
    document.getElementById('saveOriginalImage').addEventListener('click', function() {
        ...
        fetch('/Blur_Background_btn/get_image/' + originalFilename)
